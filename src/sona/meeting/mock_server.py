...
def create_contract_mock_app(fixture_dir: Path | None = None) -> FastAPI:
    ...
    app = FastAPI(title="Sona Meeting Contract Mock", version="1.0.0")
...
def main() -> None:
    ...
    parser.add_argument("--host", default=os.environ.get("SONA_MEETING_MOCK_HOST", "127.0.0.1"))
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.environ.get("SONA_MEETING_MOCK_PORT", "8200")),
    )
    parser.add_argument(
        "--fixture-dir",
        type=Path,
        default=(
            Path(os.environ["SONA_MEETING_MOCK_FIXTURE_DIR"])
            if os.environ.get("SONA_MEETING_MOCK_FIXTURE_DIR")
            else None
        ),
    )

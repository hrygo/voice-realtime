...
def _resolve_level(level: int | str | None) -> int:
    ...
    env_level = os.environ.get("SONA_LOG_LEVEL", "").strip().upper()
...
def setup_logging(
    service_name_or_level: str | int = "sona",
    *,
    ...
    Args:
        ...
        level: 日志级别（优先于环境变量 `SONA_LOG_LEVEL`）。
        log_dir: 日志输出目录（默认读取 `SONA_LOG_DIR` 或 `runtime/logs`）。
        log_file: 显式指定日志文件绝对/相对路径（优先于 `log_dir/{service_name}.log`）。
        enable_file: 是否启用文件落盘（默认读取 `SONA_LOG_TO_FILE`，默认 True）。
        enable_console: 是否启用控制台输出（默认读取 `SONA_LOG_TO_CONSOLE`，默认 True）。
    ...
    to_console = (
        _parse_bool_env("SONA_LOG_TO_CONSOLE", True)
        if enable_console is None
        else enable_console
    )
    to_file = (
        _parse_bool_env("SONA_LOG_TO_FILE", True)
    ...
    if to_file:
        if log_file is not None:
            ...
            env_dir = os.environ.get("SONA_LOG_DIR")
            target_dir = (
                Path(log_dir)

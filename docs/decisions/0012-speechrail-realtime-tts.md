---
title: "ADR-0012：TTS 运行时统一由 SpeechRail OpenAI Realtime 提供"
...
# ADR-0012：TTS 运行时统一由 SpeechRail OpenAI Realtime 提供
...
旧实现由 `sona` 维护本地/bridge TTS 进程、模型生命周期和 OpenAI 兼容转换，造成
ASR/TTS 服务边界不一致、取消与播放状态分散，以及重复的模型资源占用。SpeechRail 已提供
统一的本地 TTS OpenAI Realtime session，应用不应继续保留第二套运行时。

## 决策

- 交互管道统一使用 `SpeechRailTTSService` → `SpeechRailTTSClient`，通过 SpeechRail `/v1/realtime`
...
- REST `/v1` 仅用于音色目录与试听/回放代理；实时交互使用 `/v1/realtime`，不回退到旧 bridge。
- `scripts/run-all.sh` 只启动 `sona-ui`；SpeechRail 独立管理模型、worker、队列和健康状态。
...
- OpenAI Realtime 的 response ID、sequence 和 cancel 语义集中在一个客户端适配层，便于顺序校验和资源回收。
- TTS 播放状态仍由应用掌握，可与单一 PCM owner 及双层回声防线协调。

### 保留风险

- SpeechRail 未就绪或 profile 缺失时，语音助手必须明确失败；不能静默降级到旧 bridge。
- `/v1/realtime` session 不可透明恢复；取消或断线后应用需丢弃未播放缓存并创建新 session。

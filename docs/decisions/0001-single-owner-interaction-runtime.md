...
description: "确定 sona-ui 为交互管道唯一所有者，sona-interact 为互斥无UI替代入口，统一 InteractionSession 生命周期"
status: accepted
...
Sona 已从三个独立进程演化为 `sona-ui` 内嵌 AudioHub、Pipecat 交互管道与状态观察器，
但 `sona-interact` 仍可独立启动同一条 STT→LLM→TTS 管道。旧文档甚至要求两者同时运行，
...
- `sona-ui` 是带 UI 场景下交互管道的唯一所有者。
- `sona-interact` 作为无 UI 的 headless 替代入口保留，但与 `sona-ui` 互斥。
...
- `sona-ui` 停止交互会话时保留 AudioHub 与字幕链，但进入交互队列的音频必须暂停或丢弃；
  重新启动前清空队列，禁止回放停止期间的历史音频。

## 备选方案

### UI 仅作为外部 `sona-interact` 的控制器
...
- 原有 `sona-interact` 命令仍可使用，但不能与 `sona-ui` 并行运行。
- 启动文档、架构图、健康状态和测试应描述当前四类运行单元：`sona-ui`、SpeechRail、LM Studio

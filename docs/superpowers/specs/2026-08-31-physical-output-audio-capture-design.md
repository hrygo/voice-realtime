...
本方案已完成产品方向与架构决策确认。2026-09-05 代码已包含原生 Helper 及功能开关后的
输出单源字幕入口；真实设备门禁、热切换及双源会议尚未完成，因此状态保持为 `under_review`。
当前实现与启用方法见[验收手册](../../manuals/物理输出音频采集验收手册.md)，本文其余章节描述目标架构。

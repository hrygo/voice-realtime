...
description: "解决会议录制中单人被误识别为多人及长时停顿/语调起伏导致说话人漂移的端到端技术解决方案（Sortformer门限、CAM++声纹质心池、全局AHC聚类）"
status: active
...
> **适用范围**：`voice_realtime.meeting`、`voice_realtime.subtitles`、`WhisperLiveKit`、`PostgreSQL` 数据层与 `ui` 控制台。  
...
3. **事务安全**：在单个 PostgreSQL 事务中执行：
   ```sql
   UPDATE voice_realtime.meeting_segments 
...
2. **特征持久化**：
   后端通过 CAM++ 计算 192 维向量，存入 PostgreSQL 新建表 `voice_realtime.user_profiles`：
   ```sql
   CREATE TABLE voice_realtime.user_profiles (
...
│   1. 数据库迁移：新建 voice_realtime.user_profiles 表                           │

...
> **影响范围**：Voice Studio 语音交互助手（`vr-ui` / `vr-interact`）、Pipecat 交互管道、SpeechRail STT/TTS、回声抑制状态机、LM Studio 原生会话链

> 本文的故障发生在 SpeechRail 迁移前，原始链路名称保留在复盘中用于溯源；当前 TTS 已由
> `SpeechRailTTSService` 提供，ASR 已由 SpeechRail Realtime v2 提供。以下 EchoState 复位、取消
...
sequenceDiagram
    ...
    participant STT as SpeechRail Realtime v2 STT
...
- **核心文件**：[`src/voice_realtime/interaction/pipeline.py`](../../src/voice_realtime/interaction/pipeline.py)
- **机制**：
...
- **核心文件**：[`src/voice_realtime/interaction/reasoning.py`](../../src/voice_realtime/interaction/reasoning.py)、[`context_memory.py`](../../src/voice_realtime/interaction/context_memory.py)
- **机制**：

...
> **状态**：排查完成，待修复执行  
> **日期**：2026-08-25  
> **影响范围**：Voice Studio 语音交互助手（`vr-ui` / `vr-interact`）、Pipecat 交互管道、回声抑制状态机、LM Studio 原生会话链  
...
### 1.1 故障现象
在 Voice Studio 交互面板（耳机打断模式或外放模式）中，当 AI 助手正在进行流式语音播报（例如第 10 轮播报长句子：*“今天天气真不错，阳光暖暖的，微风轻轻吹过，让人心情特别好。”*）时，用户发声打断（插话 / Barge-in）：
...
sequenceDiagram
    ...
    participant TTS as LocalBridgeTTSService
...
### 2.1 根因一：`TTSStateObserver` 漏监听打断帧，导致 `EchoState` 永久挂死在抑制态（最直接诱因）
- **核心文件**：[`src/voice_realtime/interaction/pipeline.py`](file:///Users/hrygo/Documents/voice-realtime/src/voice_realtime/interaction/pipeline.py#L193-L212)
- **机制机制**：
...
### 2.2 根因二：`SelfEchoFilter` 语义防线在状态未复位时误杀插话文本
- **核心文件**：[`src/voice_realtime/interaction/pipeline.py`](file:///Users/hrygo/Documents/voice-realtime/src/voice_realtime/interaction/pipeline.py#L370-L410)
- **机制机制**：
...
### 2.3 根因三：LM Studio 原生会话链断裂与非交替历史恢复异常
- **核心文件**：[`src/voice_realtime/interaction/reasoning.py`](file:///Users/hrygo/Documents/voice-realtime/src/voice_realtime/interaction/reasoning.py#L471-L519)、[`context_memory.py`](file:///Users/hrygo/Documents/voice-realtime/src/voice_realtime/interaction/context_memory.py#L263-L308)
- **机制机制**：
...
### 4.1 修复 1：`TTSStateObserver` 接入打断帧监听（高优先级）

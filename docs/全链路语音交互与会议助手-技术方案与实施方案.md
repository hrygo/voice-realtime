...
1. **L1 声学能量初筛（Silero VAD）**：
   - 在 [pipeline.py](file:///Users/hrygo/Documents/voice-realtime/src/voice_realtime/interaction/pipeline.py) 中，`SileroVADAnalyzer` 实时评估 32ms 帧的语音置信度（参数：`start_secs=0.2s`, `stop_secs=0.45s`, `confidence=0.7`）。
...
4. **L4 TTS 播报分句（NLTK `punkt_tab`）**：
   - 由 [nltk_data.py](file:///Users/hrygo/Documents/voice-realtime/src/voice_realtime/interaction/nltk_data.py) 提供统计断句能力，将大模型 SSE 返回的长段落切分为小分句逐句流式送入 TTS，实现首字延迟 $< 300\text{ms}$。
...
1. **Sortformer 端到端声学分离**：
   - 模型文件：[runtime/sortformer.nemo](file:///Users/hrygo/Documents/voice-realtime/runtime/sortformer.nemo)。
...
2. **上层映射与持久化管理**：
   - [session.py](file:///Users/hrygo/Documents/voice-realtime/src/voice_realtime/meeting/session.py) 捕获段落后生成默认展示名（“说话人 0”）；
...
2. **时间戳重叠原子替换**：
   - [repository.py](file:///Users/hrygo/Documents/voice-realtime/src/voice_realtime/meeting/repository.py) 在收到新窗口时，根据 `replace_from_ms` 原子覆盖上一窗口被后文校正的时间段，保证转录时序绝对单调递增，无重复字。
...
1. **动态轮次超时策略（Dynamic Smart Turn Timeout）**：
   - 在 [pipeline.py](file:///Users/hrygo/Documents/voice-realtime/src/voice_realtime/interaction/pipeline.py) 中，动态调整 VAD 判定静音时长：
...
2. **说话人平滑滤波（Diarization Smoothing）**：
   - 在 [session.py](file:///Users/hrygo/Documents/voice-realtime/src/voice_realtime/meeting/session.py) 中增加短片段过滤：
     - 若新说话人片段时长 $< 400\text{ms}$ 且其前后为同一说话人，直接归并为原说话人；
     - 剔除孤立的环境杂音小片段。
3. **TTS 首句极速分词优化**：
   - 优化 [nltk_data.py](file:///Users/hrygo/Documents/voice-realtime/src/voice_realtime/interaction/nltk_data.py)，增加对“但是/因此/而且/包括”等连词的弱切分，确保首句字符数控制在 8~12 字送入 TTS 桥，将 TTFA 压低至 250ms 左右。
...
2. **流式标点稳定固化窗口（Stable Punctuation Commit）**：
   - 在 [subtitle_proxy.py](file:///Users/hrygo/Documents/voice-realtime/src/voice_realtime/ui/subtitle_proxy.py) 中实现本地标点对齐锁，仅在句子末尾出现确定性标点且静音间隔 $\ge 0.8\text{s}$ 时进行切段封口，消除实时字幕反复横跳。
3. **会议纪要前置 LLM 语义校对**：

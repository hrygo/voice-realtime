...
系统由 **3 个核心运行单元** 协同构成，所有服务默认监听 `127.0.0.1` 本机回环地址，纯本地离线运行：
...
flowchart TB
    ...
    subgraph SPEECHRAIL[SpeechRail · 端口 8201]
        SR[Realtime v2<br/>Qwen3-ASR / Sortformer profile / Qwen3-TTS<br/>统一 ASR/TTS session 与取消]
    end
...
| **SpeechRail ASR/TTS** | `8201` | SpeechRail Realtime v2 ASR、Sortformer profile 与 TTS；voice-realtime 只负责客户端适配、播放、回声与打断 | Qwen3-ASR / Sortformer / Qwen3-TTS |
...
麦克风 ──► [唯一共享] AudioHub (16kHz mono int16 PCM 采集 / 有界扇出)
               ...
               │
               └─► [字幕/会议共享] SubtitleProxy ──► SpeechRail Realtime v2 (ASR + Sortformer profile)
                                                        │
...
```text
麦克风 PCM ──► Silero VAD (能量帧级初筛) ──► Local Smart Turn v3 (语义完结判定) ──► SpeechRail ASR (标点断句)
```
...
3. **L3 ASR 流式滑动窗口切分**：
   - SpeechRail 按所选 ASR profile 管理滑动窗口与增量 transcription events；应用只消费标准化 partial/completed 事件。
...
1. **Sortformer 端到端声学分离**：
   - 模型文件位于 Hugging Face Hub 的项目外缓存，固定 revision 为
     `5240a64075176943f677d30fa2171c780229f341`；应用配置解析其绝对 snapshot 路径。

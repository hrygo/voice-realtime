...
sequenceDiagram
    ...
    LM-->>PG: 结构化纪要 (议题、决策项、行动项 + 原文 UUID 证据)
    PG->>PG: UPDATE meeting_minutes SET status='completed', content_json=...
...
[`MeetingSummaryService`](../../src/voice_realtime/meeting/summary.py) 在会议结束后调用 `qwen3.8-27b` 模型生成结构化纪要。为了确保纪要内容的严谨性与防幻觉，输出必须遵循严格的 JSON Schema 契约（`contracts/meeting-assistant/v1/schemas/minutes-content.schema.json`），并为每个事实点锚定转录片段的 UUID：

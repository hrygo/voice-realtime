...
> **文档性质**：系统总体架构方案与模块详细设计规范（面向工程实现与架构审计，100% 贴合代码实现）。  
> **更新时间**：2026-08-26 | **版本**：v2.1-Runtime-Arbitration
...
1. **极速人机交互（Voice Assistant）**：提供类似真人通话的全双工实时语音交互，端到端延迟 $< 600\text{ms}$，首字发音 $< 300\text{ms}$，具备智能打断（Barge-in）与双层声学防回声机制。
2. **专业会议助手（Meeting Assistant）**：支持多说话人分离（Sortformer Diarization）、实时窗口增量事务对账、PostgreSQL 结构化持久化、断电故障恢复 Journal，以及会议结束后的异步结构化纪要提取（基于本地大模型，带精确原文片段 UUID 锚定）。
...
| 原则 | 架构实现与设计考量 |
|---|---|
| **单源采集与有界扇出** | 麦克风硬件由 [`AudioHub`](../../src/voice_realtime/audio/hub.py) 独占捕获，通过内存有界队列广播至交互与字幕链路；支持真实物理静音（零音频吞吐）。 |
| **运行时模式与 PCM 单 owner** | [`RuntimeModeCoordinator`](../../src/voice_realtime/meeting/runtime_mode.py) 统一协调 `assistant`、`subtitles`、`meeting`、`idle` 四种模式，并稳定映射到 `PCMOwner.assistant/subtitles/meeting/none`。模型服务进程可以常驻预热，但任意稳定时刻只允许一个 workload 接收 PCM。 |
| **本地优先与离线封闭** | `voice-realtime` 不拥有 ASR/TTS 模型；SpeechRail 负责其本地 snapshot、离线加载与 Fail-Fast，应用侧只通过 loopback Realtime v2 消费能力，杜绝隐式联网与 SSRF 风险。 |
...
flowchart TB
    ...
    subgraph L3[3. 算法与本地模型推理层 Models & Local Inference Engine]
        ...
        ASR_M[SpeechRail Realtime v2 ASR<br/>profile-owned local snapshot]
        DIAR_M[SpeechRail Sortformer profile<br/>optional diarization]
        LLM_M1[LM Studio /api/v1/chat<br/>qwen3.6-35b-a3b 交互]
        LLM_M2[LM Studio /api/v1/chat<br/>qwen3.6-35b-a3b 纪要]
        TTS_M[SpeechRail TTS Realtime v2<br/>24kHz PCM]
    end
...
flowchart TB
    ...
    subgraph UNIT2["2. 语音服务单元: SpeechRail (:8201)"]
        SPEECHRAIL_NODE["SpeechRail Realtime v2<br/>ASR / Sortformer profile / TTS"]
    end

    subgraph UNIT3["3. 本地推理单元: LM Studio (:1234)"]
        LM_INTERACT["qwen3.6-35b-a3b (交互对话)"]
        LM_SUMMARY["qwen3.6-35b-a3b (会议纪要)"]
    end
    ...
    PROXY_NODE <==>|Realtime v2 transcription events| SPEECHRAIL_NODE
    PIPE_NODE <==>|原生 /api/v1/chat · reasoning: off| LM_INTERACT
    PIPE_NODE <==>|WS /v2/realtime · speech events| SPEECHRAIL_NODE
...
|---|---|---|---|---|
| **Voice Studio 核心宿主 (`vr-ui`)** | `8100` | [`voice_realtime.ui.server`](../../src/voice_realtime/ui/server.py) | Python 3.12 异步主进程，包含 AudioHub、InteractionSession、MeetingSession、FastAPI | HTTP REST (`/api/v1/*`, `/api/services`, `/api/runtime`, `/v1/voices`, `/v1/audio/speech`)<br>WS 控制 (`/ws/v1/control`, `/ws/assistant/cmd`)<br>WS 实时会议 (`/ws/v1/meetings`)<br>WS 字幕流 (`/ws/subtitles`)<br>WS 助手状态 (`/ws/assistant`) |
| **SpeechRail ASR/TTS** | `8201` | 外部 [`SpeechRail`](../../../SpeechRail/src/speechrail/app.py) 服务 | 独立服务负责 ASR/TTS 模型生命周期、队列、Sortformer profile、preset 与取消；voice-realtime 只持有客户端适配器 | Realtime v2 transcription/speech events<br>REST `/v1/audio/speech`、`/v1/voices` 用于试听/目录 |
...
| **PostgreSQL 数据库** | `5432` | `postgres` 守护进程 | 独立数据库服务，DSN: `postgresql:///knowledge`，Schema: `voice_realtime` | psycopg3 异步连接池 (`psycopg_pool.AsyncConnectionPool`)，严格事务隔离 |
...
[`RuntimeModeCoordinator`](../../src/voice_realtime/meeting/runtime_mode.py) 统筹运行模式切换。
...
2. **`finalizing`**：调用 `end_meeting()` 时，向 SpeechRail 发送空 PCM 作为 EOF，等待服务端冲刷完毕（默认超时 `finalization_timeout_secs = 8.0s`）；
3. **`completed`**：收到 `session.completed` 并完成收尾冲刷，会议标记为已完成，并自动向 `meeting_minutes` 插入 `queued` 状态的 AI 纪要任务；
4. **`interrupted`**：若冲刷超时（超过 8.0s）或发生不可恢复的捕获异常，会议标记为已中断并记录 `interruption_reason`（如 `finalization_timeout`）。
...
│ FINALIZING      │ 向 SpeechRail 发送空 PCM 作为 EOF；等待 `session.completed`；               │
...
[`AudioHub`](../../src/voice_realtime/audio/hub.py) 是系统的单一硬件音频采集源，负责以恒定帧长与采样率捕获麦克风信号，并通过有界队列无阻塞分发。
...
根据 [`voice_realtime.interaction.pipeline`](../../src/voice_realtime/interaction/pipeline.py)，完整的处理管道流向如下：

```mermaid
flowchart TD
    INPUT["AudioInjector / transport.input"] --> L1_ECHO["EchoSuppressionProcessor<br/>(L1 声学能量抑制)"]
    L1_ECHO --> STT["SpeechRail Realtime v2 ASR<br/>(Pipecat conversation adapter)"]
    ...
    REC --> TTS["SpeechRailTTSService<br/>(Realtime v2 :8201)"]
...
交互管道通过 [`SpeechRailTTSService`](../../src/voice_realtime/interaction/tts.py)
和 [`SpeechRailV2Transport`](../../src/voice_realtime/speechrail/transport.py) 接入独立
SpeechRail。公共模型 ID 固定为 `speechrail/qwen3-tts`，voice-realtime 不加载 TTS
权重，也不负责 TTS 进程生命周期：

1. **服务端生命周期与并发门控（Server-owned Runtime）**：
   - SpeechRail 负责模型快照、worker、队列、取消和 24kHz PCM 输出；voice-realtime
     只负责按会话创建客户端、将 PCM 映射为 Pipecat 帧，并把取消传播为 `response.cancel`。
2. **NLTK `punkt_tab` 级联流式分句**：
   - 交互管道利用 [`nltk_data.py`](../../src/voice_realtime/interaction/nltk_data.py) 将 LLM 返回的流式 Token 按标点切分为短句，首句生成即送入 TTS，实现首字发音延迟 $< 300\text{ms}$。
3. **动态音色控制**：
...
实时字幕子系统由 `vr-ui` 内的 `SubtitleProxy` 通过 SpeechRail Realtime v2（端口 `8201`）提供服务；ASR 与可选 Sortformer profile 的模型生命周期均由 SpeechRail 管理。
...
flowchart LR
    ...
    subgraph SPEECHRAIL["SpeechRail Realtime v2 (:8201)"]
        SLIDE_WIN --> QWEN_ASR["SpeechRail ASR profile<br/>增量 transcription events"]
        SLIDE_WIN --> SORTFORMER["SpeechRail Sortformer profile<br/>可选说话人分离"]
...
UI 主进程内的 [`SubtitleProxy`](../../src/voice_realtime/ui/subtitle_proxy.py) 是字幕服务与上层业务的唯一网关：
...
sequenceDiagram
    ...
    participant PG as PostgreSQL (voice_realtime Schema)
    participant RJ as RecoveryJournal (0600 JSONL)
    participant SW as MeetingSummaryService
    participant LM as LM Studio (:1234 qwen3.6-35b-a3b)
...
由于流式 ASR 的右侧上下文修正特性，已识别的语句可能会被重写。系统在 [`MeetingRepository`](../../src/voice_realtime/meeting/repository.py) 中实现了确定性的 `replace_from_ms` 窗口对账算法：
...
在 PostgreSQL 数据库发生网络抖动或临时不可写时，[`RecoveryJournal`](../../src/voice_realtime/meeting/recovery.py) 启动本地降级存储，确保数据零丢失：
...
[`MeetingSummaryService`](../../src/voice_realtime/meeting/summary.py) 在会议结束后调用 `qwen/qwen3.6-35b-a3b` 模型生成结构化纪要。为了确保纪要内容的严谨性与防幻觉，输出必须遵循严格的 JSON Schema 契约（`contracts/meeting-assistant/v1/schemas/minutes-content.schema.json`），并为每个事实点锚定转录片段的 UUID。
...
```json
{
  "overview": "会议围绕 voice-realtime 架构方案与运行模式互斥展开，确认了离线封闭与双层防回声策略。",
  "topics": [
    {
      "title": "技术架构与模式互斥评审",
...
| `GET /api/services` | HTTP REST | 进程健康与 workload 原始诊断 | `services[]` 保留 HTTP `status`；WLK additive 字段见下文；顶层 `diagnostics` 固定五键 |
| `GET /v1/voices` | HTTP REST | 代理 TTS 音色列表 | 返回可用音色 profiles |
| `POST /v1/audio/speech` | HTTP REST | SpeechRail TTS 试听/回放代理 | OpenAI 兼容接口，响应 24kHz 音频流；交互播放使用 Realtime v2 |
| `WS /ws/v1/control` | WebSocket JSON | 严格控制指令网关 | [`protocol.py`](../../src/voice_realtime/ui/protocol.py) 契约定义 |
| `WS /ws/assistant/cmd` | WebSocket JSON | 助手控制兼容通道 | [`protocol.py`](../../src/voice_realtime/ui/protocol.py) 契约定义 |
| `WS /ws/v1/meetings` | WebSocket JSON | 会议实时转录与状态流 | [`events.py`](../../src/voice_realtime/meeting/events.py) 事件广播 |
...
`GET /api/services` 中 WLK 条目新增以下 additive 字段，旧客户端可以忽略：

| 字段 | 含义 |
|---|---|
| `workload` | 当前字幕 workload 状态，例如 `paused/starting/ready/degraded/error`。 |
| `ws_state` | 应用到 WLK 的 WebSocket 原始连接状态。 |
...
```sql
-- 1. 会议元数据主表
CREATE TABLE IF NOT EXISTS voice_realtime.meetings (
    id uuid PRIMARY KEY,
...
-- 2. 说话人自定义名称映射表
CREATE TABLE IF NOT EXISTS voice_realtime.meeting_speakers (
...
CREATE TABLE IF NOT EXISTS voice_realtime.transcript_segments (
    id uuid PRIMARY KEY,
    meeting_id uuid NOT NULL REFERENCES voice_realtime.meetings(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS voice_realtime.meeting_minutes (
    id uuid PRIMARY KEY,
    meeting_id uuid NOT NULL REFERENCES voice_realtime.meetings(id) ON DELETE CASCADE,
...
-- 5. 会议事件流水审计表
CREATE TABLE IF NOT EXISTS voice_realtime.meeting_events (
    id uuid PRIMARY KEY,
    meeting_id uuid NOT NULL REFERENCES voice_realtime.meetings(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS meetings_created_at_idx
    ON voice_realtime.meetings (created_at DESC, id DESC);
CREATE INDEX IF NOT EXISTS transcript_segments_meeting_order_idx
    ON voice_realtime.transcript_segments (meeting_id, segment_order);
CREATE INDEX IF NOT EXISTS transcript_segments_meeting_start_idx
    ON voice_realtime.transcript_segments (meeting_id, start_ms);
CREATE INDEX IF NOT EXISTS transcript_segments_speaker_start_idx
    ON voice_realtime.transcript_segments (meeting_id, speaker_key, start_ms);
CREATE INDEX IF NOT EXISTS meeting_minutes_queue_idx
    ON voice_realtime.meeting_minutes (status, lease_until, created_at);
CREATE INDEX IF NOT EXISTS meeting_events_meeting_time_idx
    ON voice_realtime.meeting_events (meeting_id, occurred_at);
```
...
│ SpeechRail TTS 首包音频延迟 (TTFB)             │ < 280ms                       │
...
下列结果已于 2026-08-26 在发布候选分支重新执行；后续若有代码变更，仍须重新运行完整门禁。
...
### 发布前 localhost / LAN 真实验收（2026-08-26 已执行）
...
`subtitles → meeting → idle`、双浏览器相反命令、WLK 未 ready、活动字幕断线重连和外部高负载观察。
...
故障演练中，WLK 未 ready 时字幕和会议切换均保持 `assistant/assistant@1`；prepared meeting 保留为
`interrupted/mode_switch_aborted`。活动字幕从 `ready/connected` 进入 `degraded/backoff`，控制面继续
可用；WLK 恢复后以 `reconnect_count=5` 回到 `ready/connected`。`subtitles → meeting` 保持现有只读
...
- 本变更没有数据库 migration、模型 ID 或端口变化。`/api/services` 新字段均为 additive，旧客户端可以

...
| **实时字幕服务 (`vr-subtitles`)** | `8001` | [`voice_realtime.subtitles.launcher`](../../src/voice_realtime/subtitles/launcher.py) | 独立子进程（封装 `WhisperLiveKit`），基于 PyTorch MPS 运行 Qwen3-ASR 与 Sortformer | WebSocket Binary (接收 16kHz PCM)<br>WebSocket JSON (推 full snapshot) |
| **TTS 语音合成桥 (`vr-bridge`)** | `8765` | [`voice_realtime.tts_bridge.server`](../../src/voice_realtime/tts_bridge/server.py) | 独立子进程，基于 Apple MLX 运行 Qwen3-TTS 1.7B，单并发门串行化生成 | HTTP POST `/v1/audio/speech`<br>响应 `audio/wav` 或 `audio/x-pcm` (24kHz, int16)<br>GET `/v1/voices` |
...
`vr-subtitles`、TTS 桥和 LM Studio 等服务进程可以常驻；常驻只表示进程和模型可用，不表示其
workload 已获得麦克风数据。
...
2. **`finalizing`**：调用 `end_meeting()` 时，向 WhisperLiveKit 发送空 PCM 作为 EOF 标记，等待服务端冲刷完毕（默认超时 `finalization_timeout_secs = 8.0s`）；
3. **`completed`**：收到 `ready_to_stop` 信号或完成收尾冲刷，会议标记为已完成，并自动向 `meeting_minutes` 插入 `queued` 状态的 AI 纪要任务；
...
│ FINALIZING      │ 向 WhisperLiveKit 发送空 PCM 作为 EOF 标记；等待 ready_to_stop 信号；        │
...
[`voice_realtime.tts_bridge`](../../src/voice_realtime/tts_bridge/server.py) 将 MLX 架构的 `Qwen3-TTS` 封装为 OpenAI 兼容的 `POST /v1/audio/speech` 接口：
...
3. **动态音色控制**：
   - 支持在请求 Body 中指定 `voice` profile（如 `VoiceDesign`），或通过 `POST /v1/voice` 热切换默认音色。
...
sequenceDiagram
    ...
    MS->>WLK: 建立 prepared 会议连接并等待 ready
...
| `POST /v1/audio/speech` | HTTP REST | 代理/直接 TTS 合成 | OpenAI 兼容接口，响应 24kHz 音频流 |

...
- Realtime v2 会话不可透明恢复。`SubtitleProxy` 重连或会议重新建连时创建新会话并递增 source epoch，再由窗口对账避免重复与丢字；不能把重连描述为服务端续传。
- `scripts/run-all.sh` 只启动 `vr-ui`。SpeechRail（默认 `ws://127.0.0.1:8201/v2/realtime`）与 LM Studio（默认 `http://127.0.0.1:1234`）属于独立依赖，模型 snapshot/profile 由所属服务管理。
...
flowchart TB
    ...
    subgraph L3[3. 算法与模型服务层 Algorithms & Model Services]
        ...
        ASR_M[SpeechRail Realtime v2 ASR<br/>model/profile owned by SpeechRail]
...
flowchart TB
    ...
    subgraph UNIT2["2. 语音服务单元: SpeechRail (:8201)"]
        SPEECHRAIL_NODE["SpeechRail Realtime v2<br/>ASR / diarization profile / TTS"]
    end
...
| **SpeechRail ASR/TTS** | `8201` | 外部 [`SpeechRail`](../../../SpeechRail/src/speechrail/app.py) 服务 | 独立服务负责 ASR/TTS 模型生命周期、队列、diarization profile、preset 与取消；voice-realtime 只持有客户端适配器 | Realtime v2 transcription/speech events<br>REST `/v1/audio/speech`、`/v1/voices` 用于试听/目录 |
...
2. **`finalizing`**：调用 `end_meeting()` 时，向 SpeechRail 发送 `input_audio_buffer.commit` 作为 EOF，等待服务端冲刷完毕（默认超时 `finalization_timeout_secs = 8.0s`）；
...
2. **NLTK `punkt_tab` 级联流式分句**：
   - 交互管道利用 [`nltk_data.py`](../../src/voice_realtime/interaction/nltk_data.py) 将 LLM 返回的流式 Token 按标点切分为短句，首句生成即送入 TTS；首字发音延迟需结合当前 SpeechRail 部署重新验收。
3. **动态音色控制**：
...
实时字幕子系统由 `vr-ui` 内的 `SubtitleProxy` 通过 SpeechRail Realtime v2（端口 `8201`）提供服务；ASR 与可选 diarization profile 的模型生命周期均由 SpeechRail 管理。

### 4.1 字幕服务架构与模型集成

```mermaid
flowchart LR
    AUDIO_IN["PCM 数据流<br/>(16kHz mono int16)"] --> SR_INPUT["SpeechRail Realtime v2<br/>session.update + audio append"]
...
| `ws_state` | 应用到 SpeechRail Realtime v2 的 WebSocket 原始连接状态。 |

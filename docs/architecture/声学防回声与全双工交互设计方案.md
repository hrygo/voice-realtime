...
5. **实时字幕链路（WhisperLiveKit）** 从 `AudioHub` 接收原始麦克风 PCM，无法区分声音来自现场真人还是扬声器外放，导致外放声音被 Qwen3-ASR 转录，并被 Sortformer 聚类为 `说话人 1` / `说话人 2`。
...
flowchart TD
    ...
    subgraph S2["Layer 2: 交互引擎与自回声过滤"]
        ...
        ECHO_PROC --> STT["SenseVoice STT"]
        STT --> ECHO_FILTER["SelfEchoFilter"]
        ECHO_FILTER --> LLM["LM Studio 原生推理"]
        LLM --> TTS_BRIDGE["TTS 桥 Qwen3-TTS"]
        TTS_BRIDGE --> TTS_PLAY
    end
    
    subgraph S3["Layer 3: 实时字幕与说话人分离"]
        SP --> WLK["WhisperLiveKit :8001"]
        WLK --> QWEN_ASR["Qwen3-ASR 流式转录"]
        QWEN_ASR --> SORTFORMER["Sortformer Diarization"]
    end
...
- 在 `SubtitleProxy` 与前端数据流层：
  ...
  - 当 WhisperLiveKit 的 confirmed 文本到达时，若命中 Agent 播报时间窗口且文本相似度 $\ge 0.7$ 或为其子串：
...
- 移除底部孤立的 Agent 补充块；
- 将 WhisperLiveKit 识别的人声（`👤 说话人 X`）与 Agent 高保真回复（`🤖 AI 助手`）按物理时间戳融合为统一的剧本时间线；

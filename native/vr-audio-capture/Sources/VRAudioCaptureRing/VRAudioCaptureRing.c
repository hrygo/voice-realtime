...
#define VR_AUDIO_CAPTURE_RING_NO_READER UINT64_MAX
#define VR_AUDIO_CAPTURE_RING_MAX_CAPACITY UINT32_C(65536)
#define VR_AUDIO_CAPTURE_RING_MAX_PAYLOAD_STORAGE ((size_t)64 * 1024 * 1024)

typedef struct {
    VRAudioCaptureRingMetadata metadata;
    uint32_t payload_byte_count;
    uint8_t *payload;
} VRAudioCaptureRingSlot;

struct VRAudioCaptureRing {
    uint32_t capacity;
    uint32_t maximum_payload_bytes;
    VRAudioCaptureRingSlot *slots;
...
VRAudioCaptureRing *vr_audio_capture_ring_create(
    ...
    if (
        ...
        capacity > VR_AUDIO_CAPTURE_RING_MAX_CAPACITY ||
        (size_t)maximum_payload_bytes > VR_AUDIO_CAPTURE_RING_MAX_PAYLOAD_STORAGE ||
        (size_t)capacity >
            VR_AUDIO_CAPTURE_RING_MAX_PAYLOAD_STORAGE / maximum_payload_bytes
    ) {
        return NULL;
    }
    if ((size_t)capacity > SIZE_MAX / sizeof(VRAudioCaptureRingSlot)) {
        return NULL;
    }
    ...
    VRAudioCaptureRing *ring = calloc(1, sizeof(*ring));
    if (ring == NULL) {
        return NULL;
    }
    ...
    atomic_init(&ring->reader_index, VR_AUDIO_CAPTURE_RING_NO_READER);
...
int32_t vr_audio_capture_ring_push(
    ...
    if (
        ...
        return VR_AUDIO_CAPTURE_RING_PUSH_PAYLOAD_TOO_LARGE;
    }
    ...
    if (vr_audio_capture_ring_target_is_being_read(ring, write_index)) {
        ...
        return VR_AUDIO_CAPTURE_RING_PUSH_DROPPED_INCOMING;
    }
    ...
    VRAudioCaptureRingSlot *slot = &ring->slots[write_index % ring->capacity];
    ...
    return dropped_oldest
        ? VR_AUDIO_CAPTURE_RING_PUSH_STORED_DROPPING_OLDEST
        : VR_AUDIO_CAPTURE_RING_PUSH_STORED;
}
...
bool vr_audio_capture_ring_pop(
    ...
    for (;;) {
        ...
        if (atomic_load_explicit(&ring->read_index, memory_order_seq_cst) !=
            ...
            atomic_store_explicit(
                &ring->reader_index,
                VR_AUDIO_CAPTURE_RING_NO_READER,
                memory_order_seq_cst
            );
            continue;
        }

        VRAudioCaptureRingSlot *slot = &ring->slots[read_index % ring->capacity];
